...
return {
  new = function(self, flags)
    ...
    do
      local _obj_0 = require("kong-lapis.cmd.nginx").nginx_runner
    ...
    if flags["etlua-config"] then
      self:write_file_safe(config_path_etlua, require("kong-lapis.cmd.templates.config_etlua"))
    else
      self:write_file_safe(config_path, require("kong-lapis.cmd.templates.config"))
    end
    return self:write_file_safe("mime.types", require("kong-lapis.cmd.templates.mime_types"))
  end,
