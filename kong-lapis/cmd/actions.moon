...
import default_environment, columnize,
  parse_flags, write_file_safe from require "kong-lapis.cmd.util"
...
class Actions
  ...
  new: =>
    @path = require "kong-lapis.cmd.path"
    @path = @path\annotate!
  ...
  get_server_module: =>
    require "kong-lapis.cmd.#{@get_server_type!}"

  get_server_actions: =>
    require "kong-lapis.cmd.#{@get_server_type!}.actions"
  ...
  get_action: (name) =>
    ...
    local action
    pcall ->
      action = require "kong-lapis.cmd.actions.#{name}"
  ...
  actions: {
    {
      ...
      (flags) =>
        server_actions = if flags.cqueues
          require "kong-lapis.cmd.cqueues.actions"
        else
          require "kong-lapis.cmd.nginx.actions"

        server_actions.new @, flags

        if flags.lua
          @write_file_safe "app.lua", require "kong-lapis.cmd.templates.app_lua"
          @write_file_safe "models.lua", require "kong-lapis.cmd.templates.models_lua"
        else
          @write_file_safe "app.moon", require "kong-lapis.cmd.templates.app"
          @write_file_safe "models.moon", require "kong-lapis.cmd.templates.models"

        if flags.git
          @write_file_safe ".gitignore", require("kong-lapis.cmd.templates.gitignore") flags

        if flags.tup
          tup_files = require "kong-lapis.cmd.templates.tup"
...

    {
      ...
      (flags, environment=default_environment!) =>
        import write_config_for from require "kong-lapis.cmd.nginx"
        write_config_for environment

        import send_hup from require "kong-lapis.cmd.nginx"
        pid = send_hup!
...

    {
      ...
      =>
        import send_term from require "kong-lapis.cmd.nginx"
        pid = send_term!
        if pid
...

    {
      ...
      (flags, signal) =>
        ...
        import send_signal from require "kong-lapis.cmd.nginx"
...

    {
      ...
      (flags, code, environment=default_environment!) =>
        ...
        import attach_server, get_pid from require "kong-lapis.cmd.nginx"
...

    {
      ...
      (flags, template_name, ...) =>
        ...
        unless tpl
          tpl = require "kong-lapis.cmd.templates.#{template_name}"
...

    {
      ...
      =>
        print colors "Lapis #{require "kong-lapis.version"}"
        print "usage: lapis <action> [arguments]"

        import find_nginx from require "kong-lapis.cmd.nginx"
