local to_json
to_json = require("kong-lapis.util").to_json
local AttachedServer
AttachedServer = require("kong-lapis.cmd.attached_server").AttachedServer
...
do
  ...
  local _base_0 = {
    start = function(self, env, overrides)
      ...
      self.port = overrides and overrides.port or require("kong-lapis.config").get(env).port
      self.current_thread, self.thread_socket = assert(thread.start(function(sock, env, overrides)
        local from_json
        from_json = require("kong-lapis.util").from_json
        local push, pop
        do
          local _obj_0 = require("kong-lapis.environment")
          push, pop = _obj_0.push, _obj_0.pop
        end
        local start_server
        start_server = require("kong-lapis.cmd.cqueues").start_server
