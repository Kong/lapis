...
normalize_headers = require("kong-lapis.spec.request").normalize_headers
local ltn12 = require("ltn12")
local json = require("cjson")
local parse_query_string, encode_query_string
do
  local _obj_0 = require("kong-lapis.util")
...
do
  ...
  local _base_0 = {
    ...
    load_test_server = function(self)
      local get_free_port
      get_free_port = require("kong-lapis.cmd.util").get_free_port
  ...
  _class_0 = setmetatable({
    __init = function(self, runner)
      ...
      if not (self.runner) then
        local actions
        actions = require("kong-lapis.cmd.actions").actions
        local _exp_0 = actions:get_server_type()
        if "cqueues" == _exp_0 then
          self.runner = require("kong-lapis.cmd.cqueues").runner
        else
          self.runner = require("kong-lapis.cmd.nginx").nginx_runner
        end
      end
    end,
    __base = _base_0,
