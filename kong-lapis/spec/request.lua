...
add_cookie = function(headers, name, val)
  local escape
  escape = require("kong-lapis.util").escape
...
mock_request = function(app_cls, url, opts)
  ...
  local stack = require("kong-lapis.spec.stack")
  ...
  if opts.session then
    ...
    encode_session = require("kong-lapis.session").encode_session
  ...
  local out_headers = { }
  local nginx = require("kong-lapis.nginx")
  local buffer = { }
  local flatten
