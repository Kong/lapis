...
import normalize_headers from require "kong-lapis.spec.request"
ltn12 = require "ltn12"
json = require "cjson"

import parse_query_string, encode_query_string from require "kong-lapis.util"
...
class SpecServer
  ...
  new: (@runner) =>
    unless @runner
      import actions from require("kong-lapis.cmd.actions")
      @runner = switch actions\get_server_type!
        when "cqueues"
          require("kong-lapis.cmd.cqueues").runner
        else
          require("kong-lapis.cmd.nginx").nginx_runner

  load_test_server: =>
    import get_free_port from require "kong-lapis.cmd.util"
